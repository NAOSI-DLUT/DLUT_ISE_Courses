...
//  为1≤position≤Length()+1,position合法时返回true, 否则返回false
{
    ...
    {   // position合法
        ...
        newPtr = new Node<ElemType>(e, tmpPtr->next);// 生成薪岬?
...
// 操作结果: 进行第i趟分配
{
    for (int p = 0; p < n; ++p) { //n个数依次分配到链队列里
        ElemType temp = elem[p]; //为了取第i个关键字临时存一下
        // 下面是对整型数据使用的获取第i个关键字的方法, pow不精确
        for (int ci = 0; ci < i; ++ci) temp /= r; //把后i位切掉
        list[temp % r].Insert(list[temp % r].Length() + 1, elem[p]);
    }
}

/*
TODO:完成分配线性表数组收集的功能.
函数功能:循环遍历进行分配,当线性表不空时调用函数进行删除,重懈持翟?
参数说明:lem-元素数组,n-元素个数,r-基数,d-关键字位数,i-循环基数,list-线性表数组
返回值说明:无
*/
template <class ElemType>
...
{
    int p = 0;
    for (int i = 0; i < r; ++i) { //从0到r收集链队列
        if (!list[i].Empty()) { //如果链队列不空
            for (int j = 0; j < list[i].Length(); ++j) { //挨个元素取出来放进elem里
                list[i].GetElem(j + 1, elem[p++]);
            }
        }
    }
}
...
// 操作结果: 对elem进行基数排序
{
    LinkList<ElemType> *list = new LinkList<ElemType>[r]; //r个链队列
    for (int ci = 0; ci < d; ++ci) { //需要分配收集d次
        Distribute(elem, n, r, d, ci, list); //分配
        Colect(elem, n, r, d, ci, list); //收集
        for (int i = 0; i < r; ++i) list[i].Clear(); //清空链队列
    }
}
